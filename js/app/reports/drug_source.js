		(function () {
			define(["jquery", "d3", "jnj/chart", "common", "datatables", "datatables-colvis"], function ($, d3, jnj_chart, common) {
				...
				$(document).on('click', '#drugsource_table tbody tr', function () {
					$('#drugsource_table tbody tr.selected').removeClass('selected');
					$(this).addClass('selected');
					id = datatable.data()[datatable.row(this)[0]].concept_id;
					concept_name = datatable.data()[datatable.row(this)[0]].name;
					drug_source.drilldown(id, concept_name);
				});
				...
				drug_source.drilldown = function (concept_id, concept_name) {
					$('.drilldown svg').remove();
					$('#drugSourceDrilldownTitle').text(concept_name);
					$('#reportDrugSourceDrilldown').removeClass('hidden');

					$.ajax({
						type: "GET",
						url: getUrlFromDataCollection(page_vm.datasource(), "drugsource", concept_id),
						success: function (data) {

							// boxplots
							boxplot_helper(data.AGE_AT_FIRST_EXPOSURE, '#ageAtFirstExposureDrugSource', 'Gender', 'Age at First Exposure');
							boxplot_helper(data.DAYS_SUPPLY_DISTRIBUTION, '#daysSupplyDistributionDrugSource', 'Days Supply', 'Days');
							boxplot_helper(data.QUANTITY_DISTRIBUTION, '#quantityDistributionDrugSource', 'Quantity', 'Quantity');
							boxplot_helper(data.REFILLS_DISTRIBUTION, '#refillsDistributionDrugSource', 'Refills', 'Refills');

							// drug  type visualization
							var donut = new jnj_chart.donut();
							donut.render(common.mapConceptData(data.DRUGS_BY_TYPE), "#drugsByTypeDrugSource", 300, 200, {
								margin: {
									top: 5,
									left: 5,
									right: 200,
							...
							d3.selectAll("#reportDrugSource #drugPrevalenceByMonthDrugSource svg").remove();
							var prevalenceByMonth = new jnj_chart.line();
							// prevalenceByMonth.render(byMonthSeries, "#reportDrugSource #drugPrevalenceByMonth", 900, 250, {
							prevalenceByMonth.render(byMonthSeries, "#reportDrugSource #drugPrevalenceByMonthDrugSource", 500, 300, {
							...
							freqHistChart.render(freqHistData, "#reportDrugSource #drugFrequencyDistributionDrugSource", 500, 300, {
								xFormat: d3.format('d'),
							...
							chart.render(dataByDecile, "#reportDrugSource #trellisLinePlotDrugSource", 1000, 300, {
				...
				drug_source.render = function (datasource) {
					...
					$.ajax({
						type: "GET",
						url: getUrlFromData(datasource, "drugsource_treemap"),
						...
						success: function (data) {
							...
							datatable = $('#drugsource_table').DataTable({
								order: [ 6, 'desc' ],
								dom: 'Clfrtip',
								data: table_data,
								columns: [
							...
							treemap.render(tree, '#reportDrugSource #treemap_container', width, height, {
								onclick: function (node) {
									drug_source.drilldown(node.id, node.name)
								},
